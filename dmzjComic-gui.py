...
class DMZJComicDownloader(QWidget):
    def __init__(self, parent=None):
        super(DMZJComicDownloader, self).__init__(parent)
    ...
    def anaysisURL(self):
        ...
        try:
            if dmzj.isLegelUrl(url):
                self.url = dmzj.getmobileurl(url)
                self.comicName,self.comicIntrd,self.count,self.contentList = dmzj.getContent(self.url)

                self.contentNameList = []
                for item in self.contentList:
                        self.contentNameList.append(item[0])
                ...
                for i in range(len(self.contentNameList)):
                    self.chapterListView.addItem('{0:0>3}-{1}'.format(i+1, self.contentNameList[i]))
...
class Downloader(QThread):
    ...
    def __init__(self, selectedChapterList, comicPath, contentList, contentNameList, parent=None):
    ...
    def run(self):
        try:
            for i in self.selectedChapterList:
                outputString = '正在下载{0:0>3}-{1}...'.format(i+1, self.contentNameList[i])
                print(outputString)
                self.output.emit(outputString)
                contentPath = os.path.join(self.comicPath, '第{0:0>3}话'.format(i+1))
                ...
                if not forbiddenRE.search(self.contentNameList[i]):
                    contentPath = os.path.join(self.comicPath, '{0:0>3}-{1}'.format(i+1, self.contentNameList[i]))

                if not os.path.isdir(contentPath):
                    os.mkdir(contentPath)
                imgList = dmzj.getImgList(self.contentList[i])
                dmzj.downloadImg(self.contentList[i], imgList, contentPath)
